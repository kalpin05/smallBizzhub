...
function ClientProfile() {
  ...
  if (loading) {
    return (
      <div className="client-dashboard-container" style={{ display: 'flex', minHeight: '100vh', background: 'radial-gradient(circle at center, #1c1d3f, #050510)', color: 'white', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <div className="client-dashboard-container" style={{ display: 'flex', minHeight: '100vh', background: 'radial-gradient(circle at center, #1c1d3f, #050510)', color: 'white' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <header className="client-profile-navbar" style={{ padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleLogout} className="logout-btn" style={{ background: 'rgba(255,255,255,0.1)', border: 'none', padding: '8px 16px', borderRadius: '8px', color: 'white', cursor: 'pointer' }}>Logout</button>
        </header>
              ...
              <form className="client-profile-form" onSubmit={handleSubmit} style={{ flex: 1, minWidth: '300px' }}>
                ...
                <div className="form-group" style={{ marginBottom: '15px' }}>
                  ...
                  <input
                    ...
                    disabled
                    style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: 'rgba(255,255,255,0.5)', cursor: 'not-allowed' }}
                  />
                </div>
                ...
                <div className="form-actions" style={{ display: 'flex', gap: '15px' }}>
                  <button type="submit" className="save-btn" disabled={saving} style={{ padding: '10px 20px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>
                    {saving ? "Saving..." : "Save Changes"}
                  </button>
                  <button type="button" className="cancel-btn" onClick={handleCancel} style={{ padding: '10px 20px', background: 'transparent', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '8px', cursor: 'pointer' }}>
                    Cancel
                  </button>
                </div>
              </form>
